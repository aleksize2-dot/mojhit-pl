...
async function checkTask(taskId) {
  ...
  const paths = [
    ...
    `/api/v1/jobs/get?taskId=${taskId}`,
    `/api/v1/jobs/task-info?taskId=${taskId}`,
    `/api/v1/jobs/status?taskId=${taskId}`,
    `/api/v1/jobs/record-info?taskId=${taskId}`,
    `/api/v1/task/record-info?taskId=${taskId}`,
    `/api/v1/task/query?taskId=${taskId}`,
    `/api/v1/jobs/createTask/record-info?taskId=${taskId}`
  ];
